...
def auth():
    ...
    eutei = parse_secret(request.json.get('secret'))
    uname, password = eutei.split(b':')
    uname = uname.decode('utf-8')
    password = password.decode('utf-8')
    
    
    if uname in dummy_data.keys():
...
def rtags():
    ...
    return jsonify({'strarr': ['rest', 'email'], 'byteval': base64.b64encode(b'^[a-z0-9_]{3,8}$')})
